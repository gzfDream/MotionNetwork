...
@Time : 2019/9/30
@Author : gzf
@File : lstm_contrib.py
@brief: 使用TensorFlow中的contrib.rnn实现多层 LSTM
"""
...
class TrainConfig(object):
    ...
    num_layers = 3          # lstm层数
    num_steps = 50          # 单个数据中，序列的长度
    hidden_size = 1024      # 隐藏层中单元数目
    max_epoch = 14          # epoch<max_epoch时，lr_decay值=1,epoch>max_epoch时,lr_decay逐渐减小
    max_max_epoch = 55      # 指的是整个循环次数。
    keep_prob = 0.35        # 用于dropout.每批数据输入时神经网络中的每个单元会以1-keep_prob的概率不工作，可以防止过拟合
    lr_decay = 1 / 1.15     # 学习速率衰减
    batch_size = 64         # 每批数据的规模
...
class TrajModel(object):
    ...
    def __init__(self, is_training, config, input_):
        ...
        self.num_steps = input_.num_steps     # 序列时间步数
        size = config.hidden_size             # 隐藏层大小
        vocab_size = config.vocab_size        # 每个时刻数据维数
    ...
    def _build_rnn_graph_cudnn(self, inputs, config, is_training):
        ...
        inputs = tf.transpose(inputs, [1, 0, 2])  # 调换1,2维
        self._cell = tf.contrib.cudnn_rnn.CudnnLSTM(
            num_layers=config.num_layers,   # LSTM层数
            num_units=config.hidden_size,   # 隐藏层神经元数
            input_size=config.hidden_size,  # 输入维度
...
def get_config():
    ...
    if FLAGS.num_gpus != 1 or StrictVersion(
            tf.__version__) < StrictVersion("1.3.0"):
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        with tf.name_scope("Test"):
            test_input = TrajInput(
                config=eval_config, data=test_data, name="TestInput")
            with tf.variable_scope("Model", reuse=True, initializer=initializer):
                mtest = TrajModel(is_training=False, config=eval_config,
                                  input_=test_input)
        ...
        if StrictVersion(tf.__version__) < StrictVersion(
                "1.1.0") and FLAGS.num_gpus > 1:
            raise ValueError(
